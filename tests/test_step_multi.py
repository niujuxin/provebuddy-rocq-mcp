...
class TestStepMultiReal:
    ...
    def _reset_state_and_semaphore(self):
        ...
        core_pet._pet_semaphore = None  # reset so each asyncio.run() gets a fresh one
        yield
        _state_invalidate_all()
    ...
    def test_valid_tactics_with_mocked_pet(self):
        ...
        with patch("core.pet._ensure_pet", return_value=mock_pet):
            result = asyncio.run(
                run_step_multi(
    ...
    def test_petanque_error_with_dead_pet_returns_pet_restarted(self):
        ...
        with patch("core.pet._ensure_pet", side_effect=err):
            result = asyncio.run(
                run_step_multi(
                    tactics=["auto."],
                    lifespan_state=lifespan_state,
    ...
    def test_per_tactic_failure_tags_reason_tactic_failed(self):
        ...
        with patch("core.pet._ensure_pet", return_value=mock_alive_pet):
            result = asyncio.run(
                run_step_multi(
...
class TestStepMultiTimeoutBudget:
    ...
    def test_per_tactic_timeout_is_divided(self):
        ...
        with (
            patch("core.pet._ensure_pet", return_value=mock_pet),
            patch("core.pet._set_workspace_if_needed"),
        ):
            result = asyncio.run(
                run_step_multi(
                    tactics=[
