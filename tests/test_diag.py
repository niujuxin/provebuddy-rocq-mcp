...
from core.diag import _build_diag_snapshot
from core.envelope import _record_error
from core.pet import _run_with_pet
...
def _fast_watchdog(monkeypatch):
    monkeypatch.setattr(core_config, "_MEMORY_WATCHDOG_INTERVAL", 0.01)
...
class TestDiagSchema:
    ...
    async def test_diag_includes_server_version(self):
        ...
        Asserting against ``core.__version__`` (rather than calling
        ...
        assert snap["server_version"] == core.__version__
    ...
    async def test_max_rss_mb_threshold_reports_env_value(self, monkeypatch):
        monkeypatch.setattr(core_config, "ROCQ_MAX_PET_RSS_MB", 4242)
        ls = _fresh_lifespan_state()
        snap = _build_diag_snapshot(ls)
...
class TestRestartCounters:
    ...
    def test_first_spawn_does_not_count_as_restart(self, monkeypatch):
        ...
        core_pet._ensure_pet(ls)
        assert ls["total_spawns"] == 1
        assert core_diag._build_diag_snapshot(ls)["pet"]["restarts"] == 0
        ...
        assert core_diag._build_diag_snapshot(ls)["pet"]["restarts"] == 1
        ...
        assert core_diag._build_diag_snapshot(ls)["pet"]["restarts"] == 2

    def test_pet_generation_bumps_on_invalidate(self):
        ls = _fresh_lifespan_state()
        assert ls["pet_generation"] == 0
        core_pet._invalidate_pet(ls)
    ...
    def test_invalidate_with_active_pet_kills_and_bumps(self, monkeypatch):
        ...
        monkeypatch.setattr(core_pet, "_kill_pet", lambda p: killed.append(p))
        # Suppress the state-table invalidation hook (no-op for this test).
        monkeypatch.setattr(core_pet, "_pet_invalidation_hooks", [])
...
class TestPeakRss:
    ...
    async def test_peak_rss_updated_by_watchdog(self, monkeypatch):
        monkeypatch.setattr(core_config, "ROCQ_MAX_PET_RSS_MB", 100_000)
        _patch_psutil_rss(monkeypatch, 256)

        mock_pet = _mock_pet()
        ls = _fresh_lifespan_state()
        ls["pet_client"] = mock_pet
        monkeypatch.setattr(core_pet, "_ensure_pet", lambda lstate: mock_pet)
...
class TestRecentErrors:
    ...
    async def test_recent_errors_records_timeout(self, monkeypatch):
        monkeypatch.setattr(core_config, "ROCQ_MAX_PET_RSS_MB", 1_000_000)
    ...
    async def test_recent_errors_records_memory_abort(self, monkeypatch):
        monkeypatch.setattr(core_config, "ROCQ_MAX_PET_RSS_MB", 100)
    ...
    async def test_recent_errors_ring_buffer_caps_at_max(self):
        ls = _fresh_lifespan_state()
        cap = core_config._RECENT_ERRORS_MAX
    ...
    async def test_recent_errors_message_truncated(self):
        ...
        assert len(stored) == core_envelope._RECENT_ERROR_MESSAGE_LIMIT + len("...")
...
class TestValidationErrorsRecorded:
    ...
    async def test_run_assumptions_missing_file_recorded(self):
        from core.interactive import run_assumptions
    ...
    async def test_run_start_no_mode_recorded(self):
        from core.interactive import run_start
    ...
    async def test_run_step_multi_too_many_recorded(self):
        from core.interactive import run_step_multi
    ...
    async def test_run_notations_forbidden_recorded(self):
        from core.interactive import run_notations
...
class TestLiveStates:
    ...
    async def test_live_states_capped_at_50(self):
        ...
        ls = _fresh_lifespan_state()
        n = core_diag._DIAG_LIVE_STATES_CAP + 7  # > 50
        for i in range(n):
            add_mock_state(parent_id=None, tactic=None, step=i)
        snap = _build_diag_snapshot(ls)
        assert len(snap["live_states"]) == core_diag._DIAG_LIVE_STATES_CAP
...
class TestExtraValidationRecording:
    ...
    async def test_validation_error_records_for_run_toc(self):
        ...
        from core.interactive import run_toc
    ...
    def test_documented_reason_set_matches_expected(self):
        ...
        it)."""
        assert core_envelope._RECENT_ERROR_REASONS == frozenset(
            {
                "timeout",
                "crashed",
                "memory_exhausted",
...
class TestPetRssSampleStatus:
    ...
    async def test_pet_rss_sample_status_no_pet(self):
        ...
        rss, status = core_diag._sample_pet_rss_mb(ls)
        assert rss is None
