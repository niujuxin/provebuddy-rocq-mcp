...
class TestTruncateResult:
    ...
    def test_short_text_unchanged(self):
        from core.interactive import _truncate_result
...
class TestFeedbackCollection:
    ...
    def _setup_state_and_pet(self, fake_run):
        ...
        core_sessions._state_table.clear()
        core_sessions._state_next_id = 1

        mock_state = SimpleNamespace(st=42, proof_finished=False, feedback=[])
        sid = core_sessions._state_add(
            state=mock_state,
            file="test.v",
            theorem="test",
            workspace="/tmp",
...
class TestExtractFeedbackIncludeWarnings:
    ...
    def test_default_includes_warnings(self):
        from core.interactive import _extract_feedback
...
class TestStepMultiFeedback:
    ...
    async def test_step_multi_feedback_collected(self):
        ...
        with patch.object(core_pet, "_ensure_pet", return_value=mock_pet):
            result = await core_interactive.run_step_multi(
    ...
    async def test_default_keeps_warnings(self):
        ...
        with patch.object(core_pet, "_ensure_pet", return_value=mock_pet):
            with patch.object(
                ...
                result = await core_interactive.run_query(
                    command="Search nat",
                    preamble="",
                    workspace="/tmp",
                    lifespan_state=lifespan_state,
