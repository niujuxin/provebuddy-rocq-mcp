...
def reset_state_table():
    ...
    from core.sessions import _state_invalidate_all
    ...
    async def test_start_theorem_proof_not_finished(self):
        ...
        with tempfile.TemporaryDirectory() as ws:
            ...
            with patch.object(core_pet, "_ensure_pet", return_value=mock_pet):
                result = await core_interactive.run_start(
                    file="test.v",
                    theorem="foo",
                    workspace=ws,
                    lifespan_state=lifespan_state,
...
class TestForceRestart(_MockPetBase):
    ...
    async def test_force_restart_calls_invalidate_pet(self):
        ...
        with tempfile.TemporaryDirectory() as ws:
            ...
            with (
                patch.object(core_pet, "_invalidate_pet") as mock_invalidate,
                patch.object(core_pet, "_ensure_pet", return_value=mock_pet),
            ):
                result = await core_interactive.run_start(
                    file="test.v",
                    theorem="foo",
    ...
    async def test_force_restart_clears_state_table(self):
        ...
        with tempfile.TemporaryDirectory() as ws:
            ...
            with patch.object(core_pet, "_ensure_pet", return_value=mock_pet):
                result1 = await core_interactive.run_start(
                    file="test.v",
                    theorem="foo",
                    workspace=ws,
                    lifespan_state=lifespan_state,
            ...
            with patch.object(core_pet, "_ensure_pet", return_value=mock_pet):
                result2 = await core_interactive.run_start(
                    file="test.v",
                    theorem="foo",
                    workspace=ws,
                    lifespan_state=lifespan_state,
...
class TestStartNotFoundAvailableInFile(_MockPetBase):
    ...
    def _reset_toc_cache(self):
        from core.interactive import _TOC_CACHE
    ...
    async def test_toc_cache_hit_avoids_re_call(self, failing_pet_workspace):
        ...
        with patch.object(core_pet, "_ensure_pet", return_value=mock_pet):
            await core_interactive.run_start(
                file="test.v",
                theorem="missing1",
                workspace=str(ws),
                lifespan_state=self._lifespan_state(mock_pet),
...
class TestTruncateNames:
    ...
    def test_returns_all_when_under_limit(self):
        from core.interactive import _truncate_names
