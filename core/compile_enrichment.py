...
:mod:`core.compile` with best-effort PET state capture at the error
position.  Lives in its own module (not in ``compile.py``) so that
``compile.py`` stays free of any pytanque dependency for its core
operation.  Imports are re-pointed from upstream (rocq-mcp @ 6983113d0844c0b7f987c79dab13988445109bfb);
...
from core.compile import run_compile, run_compile_file
from core.coqc import _PROOF_FILE_LABEL, _first_error_from_positions
from core.proof_walk import ProofError, collect_file_errors
...
assert _envelope._PET_SIDE_FAILURE_REASONS <= _VALID_STATE_CAPTURE_STATUSES
assert (
    _VALID_STATE_CAPTURE_STATUSES - _NON_FAILURE_STATUSES
    == _envelope._PET_SIDE_FAILURE_REASONS
)
...
async def _capture_compile_error_state(
    ...
    *,
    ...
    try:
        from core.interactive import _state_remove, capture_position_state
    except ImportError:
...
def _enrichment_timeout(lifespan_state: dict[str, Any]) -> float:
    ...
    pet_timeout = lifespan_state.get("pet_timeout", _config.ROCQ_PET_TIMEOUT)
...
async def _multi_error_walk(
    ...
    cap = _config._COMPILE_MULTI_ERROR_CAP
    per_call_timeout = _config._COMPILE_MULTI_ERROR_TIMEOUT
    ...
    result = await _pet._run_with_pet(
        _do_walk,
        lifespan_state,
        "rocq_compile_file",
...
async def run_compile_file_with_state(
    ...
    *timing* is forwarded to :func:`run_compile_file` so the response
    ...
    if lifespan_state is not None:
        _workspace._bump_vo_epoch_if_rebuilt(
    ...
    # preserved.
    if result.get("reason") == "compile_error" and _config._COMPILE_MULTI_ERROR_CAP > 0:
