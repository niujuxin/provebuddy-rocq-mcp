"""Toolchain health & opam-switch detection.

Backing logic for the MCP ``rocq_health`` / ``rocq_switch`` tools (which
will live in our own MCP layer later).  This module is deliberately
*generic*: it reports the toolchain the running process actually resolves
on its ``PATH`` and, when the install looks like opam, the active switch.
Imports are re-pointed from upstream (rocq-mcp @ 6983113d0844c0b7f987c79dab13988445109bfb); function bodies
are byte-identical.
"""
...
import core as _core  # for __version__
...
def build_health_snapshot(lifespan_state: dict[str, Any]) -> dict[str, Any]:
    ...
    output schema.
    """
    coqc_path = _resolve_binary(_config.ROCQ_COQC_BINARY)
    ...
    if coqc_path is None:
        warnings.append(
            f"coqc binary {_config.ROCQ_COQC_BINARY!r} not found on PATH — "
    ...
    return {
        "success": True,
        "ok": ok,
        "server_version": _core.__version__,
