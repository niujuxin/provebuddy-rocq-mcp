"""Core workspace helpers: validation, path resolution, dune support, .vo epochs.

Extracted verbatim from upstream ``server.py`` sections *Shared helpers* and
*Per-workspace ".vo epoch"* (rocq-mcp @
6983113d0844c0b7f987c79dab13988445109bfb).  Imports are re-pointed to
``core.config``; all function bodies and constants are unchanged.
"""
...
def _validate_workspace(workspace: str) -> str | None:
    ...
    if _config._ROCQ_WORKSPACE_EXPLICIT:
        root = Path(_config.ROCQ_WORKSPACE).resolve()
...
def _resolve_call_timeout(
    ...
    if timeout is None or timeout <= 0:
        return None, False
    clamped = timeout > _config.ROCQ_QUERY_TIMEOUT_CAP
    return float(min(timeout, _config.ROCQ_QUERY_TIMEOUT_CAP)), clamped
...
def _find_project_root_from_file(file: str | None) -> str | None:
    ...
    try:
        ...
        if not p.is_absolute():
            p = Path(_config.ROCQ_WORKSPACE) / p
