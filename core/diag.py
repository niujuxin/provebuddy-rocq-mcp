"""Operational diagnostics — snapshot builder for the diag tool.

Backing logic for the MCP ``rocq_diag`` tool (which will live in our own
MCP layer later): pet uptime, memory headroom, live state-table entries,
and recent error history.  Imports are re-pointed from upstream
(rocq-mcp @ 6983113d0844c0b7f987c79dab13988445109bfb); function bodies are byte-identical.
"""
...
def _build_diag_snapshot(lifespan_state: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "memory": {
            ...
            "max_rss_mb_threshold": float(_config.ROCQ_MAX_PET_RSS_MB),
