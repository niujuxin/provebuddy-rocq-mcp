"""Core interactive proof tools (start, check, step_multi, query, toc, notations).

Split from upstream ``interactive.py`` (rocq-mcp @ 6983113d0844c0b7f987c79dab13988445109bfb): the ``run_*``
operations plus goal-formatting / timeout helpers.  Session bookkeeping
lives in :mod:`core.sessions`; imports are re-pointed, function bodies
...
from core.coqc import _is_focus_token, _split_rocq_sentences
from core.sessions import (
    _check_staleness,
    _get_file_end_state,
    _get_or_create_import_state,
    _reconstruct_tactic_path,
    _resolve_check_base_state,
    _state_add,
)
from core.verify import _check_forbidden_commands
...
async def run_query(
    ...
    *,
    ...
    return await _pet._run_with_pet(
        _do_query,
        lifespan_state,
        "rocq_query",
        timeout=timeout,
...
async def run_assumptions(
    ...
    *,
    ...
    """
    from core.verify import _parse_assumptions_raw, is_rocq_qualified_name
    ...
    try:
        ...
        _envelope._record_error(lifespan_state, "rocq_assumptions", msg, reason="crashed")
        return {
            "success": False,
            "reason": "crashed",
            "error": msg,
...
def _scan_module_regions(source: str) -> list[tuple[int, int, str]]:
    ...
    """
    from core.verify import _neutralize_for_regex
...
def _collect_toc_names(toc_result: Any, source: str = "") -> list[str]:
    ...
    """
    from core.verify import _NOTATION_DETAILS
...
_TRANSPORT_FAILURE_REASONS: frozenset[str] = _envelope._PET_SIDE_FAILURE_REASONS

assert _TRANSPORT_FAILURE_REASONS <= _envelope._RECENT_ERROR_REASONS, (
    ...
    f"{_TRANSPORT_FAILURE_REASONS - _envelope._RECENT_ERROR_REASONS}"
)
...
async def _fetch_available_in_file(
    ...
    *tool* is forwarded to ``_run_with_pet`` so any pet-level failure
    ...
    try:
        names = await _pet._run_with_pet(
            _do_toc,
            lifespan_state,
            tool,
        )
...
def _build_position_start_result(
    ...
    *,
    ...
    state_id = _state_add(
        ...
        vo_epoch=_workspace._current_vo_epoch(lifespan_state, workspace),
    )
    ...
    *,
    ...
    try:
        ...
        if _PetanqueError is not None and isinstance(e, _PetanqueError):
            if not _pet._pet_alive(lifespan_state.get("pet_client")):
                raise
            try:
...
async def run_start(
    ...
    if force_restart:
        _pet._invalidate_pet(lifespan_state)
...
async def run_check(
    ...
    *,
    ...
    """
    if len(body) > _config.ROCQ_MAX_SOURCE_SIZE:
        return _envelope._fail(
            lifespan_state,
            "rocq_check",
            (
                f"Body too large ({len(body)} bytes, "
                f"max {_config.ROCQ_MAX_SOURCE_SIZE})."
            ),
        )
    ...
    def _execute(pet: Any) -> dict[str, Any]:
        try:
            ...
            return {
                "success": False,
                "error": _config._PYTANQUE_NOT_INSTALLED_HINT,
            }
        ...
        _pet._set_workspace_if_needed(pet, entry_to_use.workspace, lifespan_state)
